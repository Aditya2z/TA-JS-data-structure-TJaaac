...
console.log(students.length);// This will not check the length of object, instead it will look for a key in the object with the value of variable `length` and hence will return empty value undefined because there is no key named `length` inside the object named `students`.
